        ...
        <article class="p-4 bg-white rounded-lg border border-indigo-100 flex h-full">
          <div>
            ...
            <h2 class="font-medium">
              <NuxtLink :to="{name: 'notes.show', params: {noteId: item.id}}" class="user-select-none hover:underline">{{ item.title }}</NuxtLink>
            </h2>
            <p v-if="item.text" class="mt-2">
              <NuxtLink :to="{name: 'notes.show', params: {noteId: item.id}}" class="user-select-none">{{ item.text }}</NuxtLink>
            </p>
          </div>
        </article>
      </SwiperSlide>
