<template>
  <main class="flex-auto flex flex-col" :class="{'gap-4': noGap === false}">
    ...
    <div class="flex-auto flex" :class="{'flex-row-reverse': reverse, 'gap-4': noGap === false}">
      <div class="flex-shrink-0 min-w-[560px]" :class="[!!$slots.sidebar ? 'max-w-[560px]' : 'w-full']">
        <slot></slot>
      </div>
      <div v-if="!!$slots.sidebar" class="flex-shrink-0 flex-auto" style="max-width: calc(1000px - 560px - 180px - 2rem)">
